...
hook.Add(
	...
	function()
		...
		if IsValid(ball) then
			if LocalPlayer():CanPutt() and not IsRightClickHeld then
...
function HandleControls()
	...
	if not GAMEMODE:IsPlaying() and not GAMEMODE:IsPracticing() then
		return
	end
	...
	if IsLeftClickHeld and not WasLeftClickHeld then
		-- Has PVS Issues
		if LocalPlayer():IsPocketed() and GAMEMODE:IsPlaying() then
...
function HandleAiming(ViewOrigin, ViewAngles, ViewFOV)
	local ball = LocalPlayer():GetGolfBall()
	if not IsValid(ball) or not LocalPlayer():CanPutt() then
		return
	end
	...
	if math.abs(localX) > plimitW then
		plimitW = localX
	end
	if math.abs(localY) > plimitH then
		plimitH = localY
	end
...
function GM:PlayerBindPress(ply, bind, pressed)
	...
	if bind == "+menu" then
		...
		self:DisplayScorecard(not ValidPanel(Scorecard))
	end

	if bind == "+menu_context" then
		self:DisplayCustomizer(not ValidPanel(RADIAL), true)
	end
end
