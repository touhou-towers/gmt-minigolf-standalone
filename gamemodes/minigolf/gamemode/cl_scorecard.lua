...
function SCORECARD:Paint(w, h)
	...
	table.sort(
		...
		function(a, b)
			...
			if aSwing and aFrag then
				aScore = aSwing + aFrag
			end
			if bSwing and bFrag then
				bScore = bSwing + bFrag
			end
...
function GM:DisplayScorecard(enable)
	if enable then
		if not ValidPanel(Scorecard) then
			Scorecard = vgui.Create("Scorecard")
			self:RequestScores()
		end
	else
		if Scorecard and ValidPanel(Scorecard) then
			Scorecard:Remove()
			Scorecard = nil
		end
	end
